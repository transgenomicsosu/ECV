...
command ='Rscript'
path2script ='/home/pouyaah/GEBV.R' ## path to file GEVB.R 
...
n_traits=3
epsilon=0.25
generation_number=1 ### add one more to the generations you desire
...
def call_back(model, where):
    if where == GRB.Callback.MIPSOL:
        ...
        indices = gp.tuplelist((m,k)  for m, k in model._t.keys() if vals[m, k] >= 0.9)
    ...
    if g == 0:
        with open('Error.pop','r') as file:
            ...
            for i in range(K[g]):
                ...
                for uu in range(number_of_genes):
                    if list1[uu]+list2[uu] == 0:
                        M_row.append(-1)
                    elif list1[uu]+list2[uu] == 1:
                        M_row.append(0)
                    elif list1[uu]+list2[uu] == 2:
                        M_row.append(1)                        
                M.append(M_row)
        ...
        ##writing M matrix to read in R code
        with open("M.csv",'w',newline='') as fp:
        ...
        ##writing G matrix to read in R code
        with open("G.csv",'w',newline='') as fp:
            wrt=csv.writer(fp,delimiter=',')
            wrt.writerows(G)
        ...
        os.system('cmd /c "C:/Users/Pouya Ahadi/Desktop/QL.exe"') ### path to Que Line Plus software
        ...
        ######################## R code running GEBV calculating ########################
        GEBV1=[]
        GEBV2=[]
        GEBV3=[]
        ## reading GEBV1
        input_matrix=[]
        input_matrix.append(['y','gid'])
        for tt in range(K[g]):
            input_matrix.append([pheno1[tt], tt+1])
            
        with open("Input.csv",'w',newline='') as fp:
            wrt=csv.writer(fp,delimiter=',')
            wrt.writerows(input_matrix)
        ...
        input_matrix.append(['y','gid'])
        for tt in range(K[g]):
            input_matrix.append([pheno2[tt], tt+1])
        ...
        with open("GEBV.csv") as file:
            data=file.read().split("\n")
            for rr in range(K[g]):
                GEBV2.append(float(data[rr+1])) 

        ## reading GEBV3
        input_matrix=[]
        input_matrix.append(['y','gid'])
        for tt in range(K[g]):
            input_matrix.append([pheno3[tt], tt+1])
        ...
        for q in range(cross_list[g]): 
            ...
            obj2=quicksum(t[m,k]*GEBV2[k] for k in range(K[g]) for m in range(2))
            obj3=quicksum(t[m,k]*GEBV3[k] for k in range(K[g]) for m in range(2)) 
...
## outputting summary
with open("output_GEBV.csv",'a',newline='') as fp:
    wrt=csv.writer(fp,delimiter=',')
    wrt.writerow('r')
